#!/usr/bin/env python3
"""SoundCloud DnB Waveform Analyzer — structural analysis for DnB tracks"""
import json, subprocess, sys, re
from collections import Counter

CID = "gxPRNsEq7CDD7Wvem4iymWOq3YfU7KS8"

def fetch_waveform(track, save_dir='/root/waveforms_dnb/'):
    ...
    analysis = {
        ...
        'profile_type': 'dnb' if loud_count > 8 and transitions else 'other',
...
# Load DnB tracks
with open('/root/dnb_tracks.json') as f:
    data = json.load(f)
...
# Analyze more tracks since DnB has more variety
analyze_limit = min(200, len(tracks))
for i, track in enumerate(tracks[:analyze_limit]):
    ...
    if (i+1) % 50 == 0:
        print(f"  Processed {i+1}/{analyze_limit}...", file=sys.stderr)

print(f"\n=== ANALYSIS RESULTS ===", file=sys.stderr)
print(f"Tracks analyzed: {len(results)}", file=sys.stderr)
...
outpath = '/root/dnb_analyzed.json'
